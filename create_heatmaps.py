...
from models.model_vit import mySimpleViT
...
def infer_single_slide(model, features, label, reverse_label_dict, k=1):
	...
	with torch.no_grad():
		if isinstance(model, (CLAM_SB, CLAM_MB, mySimpleViT)):
...
if __name__ == '__main__':
	...
	for i in range(len(process_stack)):
		...
		data_args.slide_ext = '.svs'
		if not os.path.exists('/ailab/user/liumianxin/HE/'+slide_name+data_args.slide_ext):
			data_args.slide_ext = '.ndpi'
		...
		if data_args.slide_ext == '.ndpi':
			wsi_ref_downsample = wsi_object.level_downsamples[patch_args.patch_level+1]
		else:
		...
		features_path = os.path.join('/ailab/group/pjlab-smarthealth03/transfers_cpfs_test/liumianxin/CLAM_DATA2/features_p2_nm/pt_files', slide_id+'.pt')
		h5_path = os.path.join('/ailab/group/pjlab-smarthealth03/transfers_cpfs_test/liumianxin/CLAM_DATA2/features_p2_nm/h5_files', slide_id+'.h5')
		...
		for sample in samples:
			if sample['sample']:
				...
				for idx, (s_coord, s_score) in enumerate(zip(sample_results['sampled_coords'], sample_results['sampled_scores'])):
					...
					if data_args.slide_ext == '.ndpi':
						patch = wsi_object.wsi.read_region(tuple(s_coord), patch_args.patch_level+1, (patch_args.patch_size, patch_args.patch_size)).convert('RGB')
					else:
		...
		if data_args.slide_ext == '.ndpi':
			...
			'custom_downsample':patch_args.custom_downsample, 'level': patch_args.patch_level+1, 'use_center_shift': heatmap_args.use_center_shift}
		else:
			wsi_kwargs = {'top_left': top_left, 'bot_right': bot_right, 'patch_size': patch_size, 'step_size': step_size, 
			'custom_downsample':patch_args.custom_downsample, 'level': patch_args.patch_level, 'use_center_shift': heatmap_args.use_center_shift}
		...
		if os.path.isfile(os.path.join(r_slide_save_dir, heatmap_save_name)):
			...
			heatmap = drawHeatmap(scores, coords, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
							thresh=-1, patch_size = vis_patch_size, convert_to_percentiles=True, blur=True)
