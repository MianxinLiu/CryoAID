...
import h5py
import math
from wsi_core.wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, save_hdf5, sample_indices, screen_coords, isBlackPatch_S, isWhitePatch, to_percentiles
import itertools
...
# sys.path.append('/mnt/data/smart_health_02/wangzirui/allslide/MDSX_Linux')
# import AllSlide
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        ...
        # self.wsi = AllSlide.AllSlide(path)
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
                            filter_params={'a_t':100}, ref_patch_size=512, exclude_ids=[], keep_ids=[], annotations=[]):
        """
    ...
    def createPatches_bag_hdf5(self, save_path, patch_level=0, patch_size=256, step_size=256, save_coord=True, annotations=None, **kwargs):
        ...
        for idx, cont in enumerate(contours):
            patch_gen = self._getPatchGenerator(cont, idx, patch_level, save_path, patch_size, step_size, annotations=annotations, **kwargs)
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        white_black=True, white_thresh=15, black_thresh=50, contour_fn='four_pt', annotations=None, use_padding=True):
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                #     patch_PIL = patch_PIL * annotations
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        contour_fn='four_pt', use_padding=True, top_left=None, bot_right=None, annotations = None):
        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.level_dim[patch_level][0], self.level_dim[patch_level][1])
        ...
        # filter coordinates
        valid = np.ones(len(results))
        for i in range(len(results)):
            x, y = results[i,0], results[i,1]
            try:
                patch_PIL = self.wsi.read_region((x,y), patch_level, (patch_size, patch_size)).convert('RGB')
                if isBlackPatch_S(np.array(patch_PIL), rgbThresh=50) or isWhitePatch(np.array(patch_PIL), satThresh=15): 
                    valid[i]=0
            except:
                valid[i]=0
                continue
