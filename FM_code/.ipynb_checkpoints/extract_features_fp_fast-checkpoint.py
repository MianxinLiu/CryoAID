...
import pandas as pd
import sys
sys.path.append('/mnt/petrelfs/yanfang/software/allslide')
import AllSlide
...
def get_wsi_handle(wsi_path):
    ...
    if postfix.lower() in ['tif', 'svs','ndpi', 'tiff', 'sdpc', 'kfb', 'mrxs', 'ndpi']:
        # handle = openslide.OpenSlide(wsi_path)
        handle = AllSlide.AllSlide(wsi_path)
...
if __name__ == '__main__':
    ...
    for index, bag_candidate_idx in enumerate(exist_idxs):
        ...
        # TCGA
        slide_file_path = all_wsi_paths[bag_candidate_idx]
