...
import sys
sys.path.append('/mnt/petrelfs/yanfang/workspace/hw/yanfang/FM_GPFM/allslide')
import AllSlide
...
class ImgReader:
    ...
    def __init__(self, filename) -> None:
        ...
        if dtype in ['tif', 'svs','ndpi', 'tiff', 'sdpc', 'kfb', 'mrxs', 'ndpi']:
            self.openslide = True
            if dtype in ['svs']:
                self.handle = openslide.OpenSlide(filename)
            else:
                self.handle = AllSlide.AllSlide(filename)
        ...
        for x_start in range(top_left[0], bot_right[0], block_size_x * int(downsample[0])):
            for y_start in range(top_left[1], bot_right[1], block_size_y * int(downsample[1])):
                ...
                if not blank_canvas:
                    ...
                    # pt = (x_start, y_start)
                    pt = (int(x_start/ int(downsample[0])), int(y_start/ int(downsample[1])))  # caution!!!
